 ...
 *
 * @param userId 登录成功用户的主键标识，返回前端时用于当前用户上下文展示。
 * @param username 登录用户名，用于前端展示和会话排查。
 * @param displayName 用户展示名称，优先用于前端昵称区域。
 * @param userType 用户类型，决定前端入口和后台权限展示。
 * @param token Sa-Token 生成的访问令牌，前端后续请求需要携带。
 */
