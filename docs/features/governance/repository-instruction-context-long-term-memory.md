...
5. 长期记忆检索按当前用户、工作空间和本轮问题读取最多 6 条 ACTIVE 记录，生成“长期记忆”上下文段。`LongTermMemoryService` 先按用户和当前 `workspaceId` 取候选，仓储范围内的 ACTIVE 记忆默认可参与回注；正文包含和关键词命中只用于把更相关的记忆排在前面，不再用硬编码项目问句或固定问句词表决定是否回注。回注成功时只打印一条“长期记忆回注已加载”汇总日志，包含 userId、workspaceId、limit、问题预览、候选数、选中数、命中 ID 与排序原因；普通列表查询日志降为 DEBUG，避免页面刷新在默认 INFO 级别反复刷屏。助手正常完成后，`GovernanceAgentContextService.extractMemoryCandidates` 委托 `LongTermMemoryService` 读取用户消息；只有文本包含“记住”“请记忆”“长期保存”“以后都按”“我的偏好”等显式授权信号时才生成 `ACTIVE` 记忆，并按范围、用户、工作空间和内容生成确定性去重键。重复内容不会再次保存，普通聊天不会被自动沉淀为长期记忆。
...
- 规范文件截断限制：单文件最多 `12,000` 字符，本轮总计最多 `24,000` 字符；成功加载日志只输出文件路径、来源类型和字节数摘要，不打印规则正文预览。
...
- 长期记忆回注排序原因：`CONTENT_OVERLAP` 表示正文与问题互相包含，`KEYWORD` 表示问题命中记忆关键词，`CURRENT_SCOPE_ACTIVE` 表示当前用户和工作空间范围内的 ACTIVE 记忆默认参与回注。查询相关性只影响排序和日志原因，不再硬过滤当前作用域记忆。

## 测试与验证

- 后端测试覆盖仓库规范文件命中、排除路径、符号链接跳过、单条汇总日志、超长内容截断、长期记忆提取去重、状态更新、检索、聊天上下文回注和用户/管理端记忆接口。
