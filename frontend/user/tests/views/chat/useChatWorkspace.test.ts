...
describe('useChatWorkspace', () => {
  ...
  /**
   * hook-notification 是后台任务 Hook 的桌面提醒通道；桌面宿主开启能力时应转交系统通知桥接。
   */
  it('收到hook-notification事件时应调用桌面系统通知桥接', async () => {
    window.localStorage.setItem(
      'codingx.auth.session',
      JSON.stringify({
        token: 'token-123',
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (input) => {
      ...
      throw new Error(`Unhandled fetch in hook notification test: ${url}`);
    });
