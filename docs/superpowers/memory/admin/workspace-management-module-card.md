...
- 管理端工作空间详情页面向“这个空间里有什么会话”的排查场景，按 `workspace_id` 展示会话分页并链接到现有会话详情。
...
- `AdminWorkspaceController` 暴露 `/api/admin/workspaces` 和 `/api/admin/workspaces/{workspaceId}/conversations` 两个只读入口。
- `ChatConversationRepository.findAllByWorkspaceId` 是管理端空间详情页的仓储入口，只读查询 `deleted = 0` 且绑定目标 `workspace_id` 的会话。
...
- 工作空间详情页不能把路由中的工作空间 ID 转为 JavaScript `Number`，数据库雪花 ID 超过安全整数范围时必须保持字符串传参。
...
- 如需扩展空间详情页展示消息摘要、工具调用或运行记录，优先复用已有会话详情/Trace 查询入口，避免在工作空间模块重复聚合消息明细。
...
- 不要用 `Number(workspaceId)`、`parseInt(workspaceId)` 等方式处理路由中的雪花 ID；浏览器会静默丢精度，典型表现是列表可见但详情接口返回“工作空间不存在”。
