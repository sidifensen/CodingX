...
| 截图 3 | 文件选项卡顶部有路径面包屑，右侧有目录树，左侧显示文件内容 | 文件 tab 顶部直接显示选中文件完整路径（未选择时显示“未选择文件”），右侧目录来自桌面宿主 `listDirectory`，点击文件后左侧只读文本铺满内容区，不再包裹卡片边框 |
...
4. 文件 tab 先从当前聊天 workspace 获取 `workspacePath`。顶部标题区直接展示当前选中文件的完整路径，未选择文件时显示“未选择文件”占位。桌面端通过 `resolveHostBridge().listDirectory(currentDirectory)` 读取目录项，并按目录优先排序；用户点击目录会进入该目录，点击文件时通过 `readTextFile(path)` 获取只读文本内容，正文直接铺满内容区，不再使用边框卡片包裹。主进程只允许读取已绑定仓库内的普通文件，且单文件预览限制为 1MB；如果旧桌面进程缺少 `host:read-text-file` handler，前端会提示用户重启 CodingX，而不是直接展示 Electron 原始报错。
...
- `frontend/user/src/views/chat/workbench/RightWorkbench.tsx`：右侧工作台容器，包含选项卡管理、宽度拖拽、添加菜单、默认启动器（功能卡片带内间距）和默认宽度常量。
- `frontend/user/src/views/chat/workbench/FileWorkbenchPanel.tsx`：文件面板，顶部展示选中文件完整路径，正文铺满展示，含目录读取和桌面端 IPC 错误转译。
