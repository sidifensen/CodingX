...
10. 用户在 `Textarea` 输入内容时，`CodingXTuiModel.view()` 会把 composer 稳定渲染为单行 `› 输入内容█`，空输入时只把 placeholder 文案置灰，块状光标保持可见并跟随 tui4j blink 消息闪烁；当输入以 `/` 开头时，模型在输入区上方渲染纯文本命令面板。命令面板会按斜杠后的关键字过滤本地命令和后端启用命令，普通候选用灰色弱提示，当前候选用 `›`、蓝色命令名和灰底标记；每次新增或删除输入字符时选中索引都会回到第一项，避免旧索引落到新的过滤结果上。面板打开时上下键只在候选列表内循环移动，不再交给输入框移动光标；Tab 或 Enter 会先把当前高亮命令填入输入框，如果输入已经是完整本地命令，Enter 才继续走命令提交。用户提交后，模型先判断是否为本地 Slash Command：`/login` 调用 `CliAuthService.loginWithBrowser()` 打开浏览器登录，`/logout` 调用 `CliAuthService.logout()` 清理本机 token，`/help` 或 `/` 只展示命令列表；未知命令会展示中文错误和可用命令，不会进入后端聊天流。
