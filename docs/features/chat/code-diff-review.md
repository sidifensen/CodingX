...
聊天页和 desktop 端在 AI 调用本地文件工具写代码时，会实时展示正在编辑和已编辑的文件差异。用户可以在助手消息内点击文件查看 diff 弹窗，也可以通过右侧代码审查栏切换本轮编辑、上轮对话、未暂存、已暂存、提交和分支等差异来源。
...
3. `ChatView` 收集助手消息过程卡片中的 `fileDiffs`，在消息内渲染编辑文件摘要。用户点击文件行时打开自定义 `FileDiffDialog`，弹窗内使用 `DiffTextBlock` 对 `+`、`-`、hunk 和普通行做分色展示，不使用浏览器原生弹窗。
...
5. 侧栏切到“上轮对话”时只读取上一条带差异的助手消息。切到“未暂存”“已暂存”“提交”或“分支”时，前端通过 `ChatApi.invokeTool(token, 'git_diff', { mode })` 请求当前 workspace 差异，并用同一套文件列表和 diff 视图展示结果。
6. `git_diff` 工具只在已绑定工作区内执行 git 命令。未检测到差异时返回空 `fileDiffs` 和中文提示；检测到差异时解析 unified diff，按文件路径、增删行数和完整 diff 文本返回给前端。
...
- `backend/src/main/java/com/codingx/tool/application/service/ChatToolUserService.java`：把 `git_diff` 纳入聊天工具调用白名单。
...
- `frontend/user/src/views/ChatView.tsx`：渲染消息内编辑文件摘要、diff 弹窗和右侧代码审查栏。
- `frontend/user/src/views/chat/chatApi.ts`：提供统一工具调用入口，供右侧栏读取工作区 git diff。
...
- desktop 端绑定 `D:\code\test` 后发送写代码请求，验证工具执行期间出现“正在编辑”，完成后消息区和右侧栏均展示文件差异，并保存浏览器/CDP 截图证据。
