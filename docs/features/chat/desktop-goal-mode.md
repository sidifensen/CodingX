...
聊天页右侧会在目标模式开启、正在流式生成或已有执行步骤时显示悬浮目标进度窗。该窗口不是新的后端目标查询接口，而是基于当前会话已有的 `executionSteps` 和流式状态生成轻量进度视图。
...
3. 后端 `ChatStreamRequestApplicationService` 继续按已有 `planMode` 字段构造 `SendChatMessageCommand`。`ChatApplicationService.buildPlanModeContext` 将系统提示扩展为 CLI 或桌面端通用的“规划/目标模式”，要求模型先判断用户是要方案还是要执行。
...
- `backend/src/main/java/com/codingx/chat/application/service/chat/ChatApplicationService.java`：生成规划/目标模式系统提示，引导目标工具调用。
- `backend/src/test/java/com/codingx/chat/application/service/ChatApplicationServiceTest.java`：验证目标模式提示包含桌面端和目标工具约束。
