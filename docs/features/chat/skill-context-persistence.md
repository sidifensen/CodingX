...
5. 聊天主流程继续按改写结果进入意图识别、模型调用和工具白名单校验；即便用户只问“这是啥”，也会保存真实助手消息与 `chat_execution_run.intent_code`，不会再落库 `skill.intro` 这类伪完成状态。短句技能说明不会向模型暴露本地工具 schema，避免进入工具调用模式后把正文缓冲到轮次结束才一次性推给前端。
...
由于入模用户正文会剥离开头 `@skill`，技能系统提示必须补足指代关系：带有明确目标的 `@web-access 帮我看 https://example.com` 会保留技能上下文进入模型；`@web-access 这是啥` 这类短句会先解释当前引用的技能本身，不能伪造成已经执行联网操作。短句说明只需要模型根据技能上下文作答，不需要读取本地文件、浏览器或网页，因此 `ChatApplicationService` 会隐藏本地工具列表并走普通流式通道，让 `assistant.delta` 立即推送到前端。多个技能同时选中且用户提供了可对比对象时，“这个和这个有什么区别”默认按已选技能列表进行解释或对比；如果用户要求执行技能任务但缺少 URL、当前页面、搜索词、附件或其他必要目标，模型再围绕已选技能追问缺失信息。
...
- `mvn -Dtest=ChatApplicationSearchFlowTest#sendMessageWithWebAccessSkillStillUsesSystemSearchEvidence,ChatApplicationServiceTest#sendMessageRoutesSelectedSkillShortQuestionThroughModelInsteadOfStaticIntro test`
