 ...
 * @param runId 消息所属执行记录标识，未绑定运行记录时为空。
 ...
 * @param deleted 逻辑删除标记，0 表示可见、1 表示已删除。
 * @param createdAt 消息创建时间，用于历史消息排序和展示。
 * @param updatedAt 消息更新时间，用于管理端排障核对持久化状态。
...
public record ChatMessageResponse(
    ...
    Long runId, // 消息所属执行记录标识，未绑定运行记录时为空。
    ...
    Integer deleted, // 逻辑删除标记，0 表示可见、1 表示已删除。
    LocalDateTime createdAt, // 消息创建时间，用于历史消息排序和展示。
    LocalDateTime updatedAt, // 消息更新时间，用于管理端排障核对持久化状态。
