...
- `AutomationTaskScheduler` 已按固定间隔扫描到期启用任务，并把 `lastRunAt`、`lastRunStatus` 和 `nextRunAt` 推进委派给服务层；仓储写入使用旧 `nextRunAt` 条件做乐观认领，避免重叠扫描重复触发同一任务。认领成功后会调用 `AutomationTaskExecutionService`，把任务 prompt 派发为后台聊天运行。
- `AutomationTaskExecutionService` 负责解析结果交付会话：会话创建任务复用来源会话，手动任务首次执行时创建 `自动化任务：<任务名>` 会话并写回 `sourceConversationId`。真实搜索、模型输出、SSE 完成事件和未读提醒全部复用 `ChatStreamExecutionService`。
...
- `backend/src/main/java/com/codingx/automation/application/service/AutomationTaskExecutionService.java`：已认领任务派发到聊天后台执行链路。
...
- 调度器只负责扫描和推进状态，不在调度层拼装聊天请求或重复实现任务筛选规则；真实执行只能消费成功认领后的触发快照，并通过 `AutomationTaskExecutionService` 统一派发。

## Extension Points

- 任务启停、删除、编辑和执行历史仍待扩展，必须继续按任务归属校验并返回统一中文 `ApiResponse.message`。
- 调度触发后的真实 AI 执行已复用聊天后台执行服务；后续仍可继续补充执行历史、重试保护和更细粒度失败原因。
...
- 不要把调度状态推进误当成真实任务执行完成；当前 `TRIGGERED` 表示已被扫描认领并开始派发，最终结果以聊天助手消息和会话未读提醒为准。
- 不要让执行服务绕过聊天主链路直接调用模型；自动化应复用现有聊天搜索、模型、Trace、SSE 和未读提醒能力。
