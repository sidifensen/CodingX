...
提供 CodingX 独立 CLI 的 Codex 风格 TUI 终端入口，让开发者输入 `codingx` 后在普通终端屏幕中进入交互模式：shell 中的 `codingx` 命令上下文会保留，启动页只展示紧凑信息卡、Tip、单行输入提示和简洁状态行。当前版本已经把 TUI 输入对接到现有后端 `/api/chat/stream` SSE 对话流，并新增浏览器登录授权：CLI 会在本机启动临时 loopback 回调服务，浏览器访问用户端 `/cli-login` 完成账号确认，随后 CLI 把一次性授权码兑换为 satoken 并保存到用户主目录配置；浏览器回调成功页会以 CodingX CLI 连接完成状态提示用户关闭页面并回到终端。终端交互层按 `› 用户输入` 和 `• AI 回复` 区分说话者，AI 回复前保留一行间距，运行中 `Working` 等待提示使用灰色弱提示，输入框自绘块状光标会跟随左右和上下方向键移动；Esc 只用于中断当前 AI 回复或取消浏览器登录等待，空闲状态不会退出 TUI。
...
- `codingx auth login`：默认登录入口，CLI 在 `127.0.0.1:<randomPort>/callback` 启动临时回调服务，打开浏览器进入用户端 `/cli-login`，授权成功后用一次性 code 兑换 satoken 并写入 `.codingx/cli.yml`；浏览器完成页展示 `CodingX CLI 已连接`、终端命令提示和关闭页面指引，并支持系统暗色主题。
...
- `codingx`：默认进入 TUI 交互界面，顶部展示 `>_ CodingX CLI (v0.1.0)` 启动卡片、后端选模提示和当前工作区；任务在带块状闪烁光标的 `› █输入任务，/ 查看命令` 输入行里提交，并通过后端聊天流执行。输入框光标不是固定假光标，而是读取 tui4j `Textarea` 的真实行列状态，用户可以用左右方向键修改前后字符，也可以在粘贴多行文本后用上下方向键切换编辑位置。输入 `/` 会在输入区上方展示 `/login`、`/logout`、`/help` 和后端启用命令候选，面板默认选中第一项，用户可用上下键切换候选；Tab 只把高亮命令填入输入框，Enter 会直接执行本地命令或发送后端命令。未选中的候选保持灰色弱提示，当前选中项只改变整行文字颜色，不使用背景块。CLI 不再展示没有真实来源的模型名、MCP 连接数或工具数量。
...
10. 用户在 `Textarea` 输入内容时，`CodingXTuiModel.view()` 会把 composer 稳定渲染为单行 `› 输入内容█`，空输入时只把 placeholder 文案置灰，块状光标保持可见并跟随 tui4j blink 消息闪烁；当输入以 `/` 开头时，模型在输入区上方渲染纯文本命令面板。命令面板会按斜杠后的关键字过滤本地命令和后端启用命令，普通候选用灰色弱提示，当前候选用 `›` 和蓝色整行文字标记；每次新增或删除输入字符时选中索引都会回到第一项，避免旧索引落到新的过滤结果上。面板打开时上下键只在候选列表内循环移动，不再交给输入框移动光标；Tab 只把当前高亮命令填入输入框并保留编辑状态，Enter 会先把当前高亮命令落入输入框，再复用原提交链路直接执行本地命令或发送后端命令。提交本地命令时，模型判断 `/login` 调用 `CliAuthService.loginWithBrowser()` 打开浏览器登录，`/logout` 调用 `CliAuthService.logout()` 清理本机 token，`/help` 或 `/` 只展示命令列表；未知命令会展示中文错误和可用命令，不会进入后端聊天流。
...
- `cli/src/main/java/com/codingx/cli/tui/CodingXTuiModel.java`：TUI 状态模型，维护启动 banner、事件窗口、输入框、Slash Command 面板、任务提交前登录检查、活动助手回答块、运行中等待提示、Esc 中断、计划模式和状态栏；当前轮输出过长时按终端宽度固定展示最新用户问题。最终视图会先按终端宽度预换行并统一 LF，底部 composer 使用带块状闪烁光标的稳定单行渲染，用户消息、AI 消息、输入框和状态栏保持紧凑衔接，避免 Windows Terminal 自动换行或 tui4j 尾部裁剪造成 renderer 刷新错位。
...
- `CodingXTuiModel`：TUI 内存状态，包含 `Viewport` 事件窗口、`Textarea` 任务输入框、当前工作区、活动助手回答块、`Plan mode` / `Chat mode`、运行状态、最近一次用户问题、运行计时、当前流式 `Future`、轮次序号、终端宽高和后端流消费线程；提交任务时会把当前模式传给事件源。UI 状态只在进程内维护，会话续接状态写入用户主目录 CLI 配置。普通屏幕可见区按终端视觉行计算，不只按 `\n` 逻辑行计算，避免单行长回复自动换行后继续隐藏用户问题；运行中的灰色 `• Working (Ns • esc to interrupt)` 只作为派生渲染行参与视图和高度估算，完成、错误或中断后不会残留。最终 `view()` 输出会预换行并统一 LF，底部 composer 只渲染一行，并把 `Textarea.line()` 与 `lineInfo().charOffset()` 转换为自绘块状光标位置，保证 tui4j 标准 renderer 的光标回退行数、尾部裁剪和方向键编辑都不会吞掉用户消息。
...
- `cd cli && mvn -Dtest=CodingXTuiModelTest test`：覆盖用户问题可见性、完成后问题锚定、底部三行尾部裁剪、TTY 单行 composer、LF-only renderer 视图、运行中输入保留、连续问答顺序、用户输入与 AI 回复间距、灰色 Working 等待提示、方向键移动输入光标、长输出裁剪、长单行自动换行、视图预换行，以及真实 `Program.run()` + `program.send` 流式增量时底部可见用户问题的场景。
- `cd cli && mvn package`
