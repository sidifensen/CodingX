...
- `codingx`：默认进入 TUI 交互界面，顶部展示 `>_ CodingX CLI (v0.1.0)` 启动卡片、后端选模提示和当前工作区；任务在带块状闪烁光标的 `› █输入任务，/ 查看命令` 输入行里提交，并通过后端聊天流执行。输入框光标不是固定假光标，而是读取 tui4j `Textarea` 的真实行列状态，用户可以用左右方向键修改前后字符，也可以在粘贴多行文本后用上下方向键切换编辑位置。输入 `/` 会在输入区上方展示 `/login`、`/logout`、`/help` 命令候选，命令由 TUI 本地消费，不会发送到聊天接口。CLI 不再展示没有真实来源的模型名、MCP 连接数或工具数量。
...
10. 用户在 `Textarea` 输入内容时，`CodingXTuiModel.view()` 会把 composer 稳定渲染为单行 `› 输入内容█`，空输入时只把 placeholder 文案置灰，块状光标保持可见并跟随 tui4j blink 消息闪烁；当输入以 `/` 开头时，模型在输入区上方渲染纯文本命令面板。用户按 Enter 后，模型先判断是否为本地 Slash Command：`/login` 调用 `CliAuthService.loginWithBrowser()` 打开浏览器登录，`/logout` 调用 `CliAuthService.logout()` 清理本机 token，`/help` 或 `/` 只展示命令列表；未知命令会展示中文错误和可用命令，不会进入后端聊天流。
