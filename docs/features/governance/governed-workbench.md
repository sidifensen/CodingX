...
软件端治理工作台用于把桌面端和聊天工作区中的开发任务能力纳入可配置、可审计的治理链路。当前实现覆盖 Plan/Checklist 步骤落库、内置 Slash Command、工具权限策略、Hook 审计、项目画像扫描和管理端治理中心；长期记忆、真实 SubAgent/Worktree 并行隔离、Agent Teams 不属于本次已实现范围。

## 使用入口

- 用户端聊天输入区：输入 `/` 打开 Slash Command 面板，可选择 `/review`、`/fix-test`、`/commit`、`/generate-doc` 等启用命令。
- 管理端：侧边栏进入「治理中心」，可维护权限策略、Hook 规则、项目画像和 Slash Command，并查看权限审计与 Hook 审计。
...
5. 管理端治理中心加载策略、Hook、项目画像、Slash Command 和审计数据；编辑操作使用 Ant Design Modal，接口错误直接展示后端 `ApiResponse.message`。项目画像扫描接收工作空间 ID 和路径，由后端扫描 Maven、NPM、Vite 等仓库标记并保存摘要、技术栈和建议验证命令。
...
- `frontend/admin/src/pages/GovernanceCenterPage.tsx`：管理端治理中心页面。
...
- `governance_project_profile`：项目画像扫描结果，记录工作空间路径、技术栈、入口和验证命令。
